...
contract TokenDistributor is ReentrancyGuard {
    ...
    /**
     * @notice Constructor
     * @param _TestToken LOOKS token address
    ...
    constructor(
        ...
        uint256 nonCirculatingSupply = ITestToken(_TestToken).SUPPLY_CAP() -
            ITestToken(_TestToken).totalSupply();
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer LOOKS tokens to the sender
        TestToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = TestToken.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }

            TestToken.mint(tokenSplitter, tokenRewardForOthers);
        }
