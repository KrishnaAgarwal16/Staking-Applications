...
import {TokenDistributor} from "contracts/LooksRare/TokenDistributor.sol";
...
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    /**
     * @notice Constructor
     * @param _TestToken address of the token staked (LOOKS)
     ...
     * @param amount amount to deposit (in TEST)
    ...
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        require(amount > 0, "Deposit: Amount must be >= 1 TEST");
    ...
    function _checkAndAdjustLOOKSTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (TestToken.allowance(address(this), _to) < _amount) {
            TestToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        TestToken.safeTransfer(msg.sender, currentAmount);
